 ...
 */
    .controller('AppCtrl', function ($scope, $ionicModal, $timeout, $localstorage, $ionicPush, $location, rAPI, $tool_fn) {
        ...
        var _checkUserLogin = function () {
            ...
            if ($tool_fn._isEmpty($scope.user)) {
                return;
            }
        ...
        function ($scope, $rootScope, $location, $stateParams, $ionicPush, HandbookService, SectionService,
            ...
            if ($scope.user || (typeof $scope.user == 'object' && $scope.user.username)) {
                ...
                if (typeof $scope.org._links.logo == 'object' && $scope.org._links.logo.href) {
                    rAPI.get($scope.user.user.session_key, $scope.org._links.logo.href).then(function (res) {

                        if (typeof res == 'object' && res.status == 200) {
                            //console.log(res.data._links);
                            rAPI.get($scope.user.user.session_key, config.path.baseURL + res.data._links.url.href).then(function (res) {
                ...
                if ($scope.org._links.handbooks) {
                    ...
                    console.log($scope.user);
                    rAPI.get($scope.user.user.session_key, $scope.org._links.handbooks.href + ony_active).then(function (return_data) {

                        $scope.handbooks = [];
                        $ionicLoading.hide();
                        if (typeof return_data.data != 'object' || !return_data.data._embedded) {
                            return;
                        }
                        ...
                        if (updateCache) {
                            ...
                            angular.forEach($scope.handbooks._embedded.items, function (item, i) {
                                ...
                                rAPI.get($scope.user.user.session_key, item._links.translations.href).then(function (res) {
                                    if (typeof res == 'object' && res.status == 200) {
    ...
    .controller('HandbookCtrl', function ($scope, $rootScope, $location, $stateParams,
        ...
        $scope.openModalPDF = function (afile) {
            ...
            function showModal(bfile) {
                ...
                $scope.pdfUrl = getCacheDir() + bfile.pdf_info.provider_reference;
                // alert($scope.pdfUrl);
                $scope.modal.show();
                $scope.tmodal = {
                    "type": "pdf",
            ...
            if ($window.cordova) {
                ...
                $ionicPlatform
                    .ready(function () {
                        ...
                        $cordovaFile.checkFile(getCacheDir(), afile.pdf_info.provider_reference)
                            .then(function (success) {
                                ...
                                //  DOWNLOAD FILE
                                $cordovaFileTransfer.download(afile.pdf_file.url, getCacheDir() + afile.pdf_info.provider_reference, {}, true)
        ...
        if ($scope.user || (typeof $scope.user == 'object' && $scope.user.username)) {
            ...
            // GET HANDBOOK
            rAPI.get($scope.user.user.session_key, $scope.org._links.handbooks.href + "/" + $scope.handbook_id).then(function (return_data) {
                $scope.handbook = return_data.data;
                $scope.ch_color = '#' + 'cfae79';
                $ionicLoading.hide();
                //console.log($_handbook);
                if (($_handbook && $_handbook.version == $scope.handbook.version && $_handbook.lang) || (typeof $_handbook == "object" && $_handbook.version && $_handbook.version == $scope.handbook.version)) {
                ...
                // GET HANDBOOK LANG
                rAPI.get($scope.user.user.session_key, $scope.handbook._links.translations.href).then(function (res) {
                    if (typeof res == 'object' && res.status == 200) {
                ...
                if (($local_handbook && $local_handbook.version == $scope.handbook.version) || (typeof $local_handbook == "object" && $local_handbook.version && $local_handbook.version == $scope.handbook.version)) {
                    $ionicLoading.hide();
                    $scope.sections = $local_handbook.data;
                    ...
                    rAPI.get($scope.user.user.session_key, $scope.handbook._links.sections.href + "?search=section.parent{null}1&limit=500").then(function (return_data) {
                        $ionicLoading.hide();
                        ...
                        angular.forEach(return_data.data._embedded.items, function (item, i) {
                            ...
                            (function (itemInstance) {
                                ...
                                // LOAD LANG
                                rAPI.get($scope.user.user.session_key, itemInstance._links.translations.href).then(function (res) {
                                    if (typeof res == 'object' && res.status == 200) {
                                ...
                                // LOAD CONTENT
                                rAPI.get($scope.user.user.session_key, itemInstance._links.contents.href + "?sort=content.ordering:asc").then(function (res) {
                                    if (typeof res == 'object' && res.status == 200 && res.data.total != 0) {
                                        $scope.sections[i]['contents'] = new Array();

                                        angular.forEach(res.data._embedded.items, function (item, key) {
                                            if (item.html_text === 'none') {

                                                rAPI.get($scope.user.user.session_key, item._links.image_url.href + '?locale=en_us').then(function (res) {
                                                    var content_data = {
                                                        type: 'image',
                                            ...
                                            } else if (item.html_text === 'none_pdf') {
                                                rAPI.get($scope.user.user.session_key, item._links.pdf.href + '?locale=en_us').then(function (res) {
                                                    if (!res.data) {
                                                        return;
                                                    }

                                                    // GET PDF URL
                                                    rAPI.get($scope.user.user.session_key, config.path.baseURL + res.data._links.url.href).then(function (res2) {
                                                        var content_data = {
                                                            type: 'pdf',
                                                            ordering: item.ordering,
                                                            pdf_info: res.data,
        ...
        _loadChildSection = function ($section) {
            ...
            if ($section._links.children) {
                ...
                $ionicLoading.show();
                rAPI.get($scope.user.user.session_key, $section._links.children.href).then(function (res) {
                    $ionicLoading.hide();
                    ...
                    angular.forEach(res.data._embedded.items, function (item, k) {
                        (function (itemInstance) {
                            ...
                            rAPI.get($scope.user.user.session_key, itemInstance._links.contents.href + "?sort=content.ordering:asc").then(function (res) {
                                if (typeof res == 'object' && res.status == 200 && res.data.total != 0) {
                                    $scope.sections[j].children._embedded.items[k]['contents'] = new Array();
        ...
        function ($scope, $rootScope, $location, $stateParams,
            ...
            if (!$scope.user) {
                ...
                _GetContactAPI = function () {
                    // body...
                    $scope.ch_color = '#' + 'cfae79';
                    rAPI.get($scope.user.user.session_key, $scope.org._links.positions.href + "?search=position.handbookContact:1,position.enabled:1").then(function (contact_res) {
                        ...
                        if (data._embedded.items.length > 0) {
                            ...
                            angular.forEach(data._embedded.items, function (item, i) {
                                $scope.contacts[i] = {};
                                rAPI.get($scope.user.user.session_key, item._links.employee.href).then(function (res) {
                                    ...
                                    if (item._links.employee_classes) {
                                        rAPI.get($scope.user.user.session_key, item._links.employee_classes.href).then(function (res) {
                                    ...
                                    if (item._links.employee_classes) {
                                        rAPI.get($scope.user.user.session_key, item._links.employee_functions.href).then(function (res) {
    ...
    .controller('NotificationCtrl',
        function ($scope, $rootScope, $location, $stateParams, rAPI, $localstorage, $ionicLoading, $ionicPush, OrgService, ImgService) {
            ...
            if (!$scope.user) {
                ...
                rAPI.get($scope.user.user.session_key, $scope.user.user._links.messages.href + var_filter).then(function (res) {
                    $ionicLoading.hide();
                    if (typeof res == 'object' && res.status == 200) {
                        $ionicLoading.hide();
